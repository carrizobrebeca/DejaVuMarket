    ...
    return carrito.reduce((acc, item) => {
      if (item.nombre === "Empanadas") {
        if (item.cantidad === 6) {
          ...
          return acc + 1000; // Precio total para 12 empanadas
        }
      }
      // Para otros productos, multiplicar por el precio y la cantidad
  ...
  const createPreference = async () => {
    try {
      const response = await axios.post(
        "https://dejavumarket-back-production.up.railway.app/create_preference",
        {
          title: "Compra",
          quantity: 1,
          price: total,
  ...
  return (
    <>
      <nav className="bg-black text-gray-800 p-4 flex justify-between items-center shadow-lg sticky top-0 z-50">
        ...
        <div
          ref={sidebarRef}
          className={`fixed top-0 right-1 h-full w-74 bg-black border-2 border-yellow-500 text-white transition-transform transform ${
                ...
                <div>
                  ...
                  <div className="mt-4">
                    <h3 className="text-xl font-bold">Total: ${total}</h3>
                      ...
                      <button onClick={handleBuy} className="mt-4 p-2 bg-blue-500 text-white rounded">
                        Comprar
                      </button>
